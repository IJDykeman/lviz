...
static void key_callback(GLFWwindow *window, int key, int scancode, int action, int mods) {
  if (action == GLFW_PRESS) {
    ...
    if (key == GLFW_KEY_Z) cursor.set_camera_translation_from_cursor_normalized(Eigen::Vector3f(.00001, 0, 1).normalized());

    if (key == GLFW_KEY_R)
          cursor.set_camera_translation_from_cursor_normalized(-cursor.camera_translation_from_cursor_normalized);
